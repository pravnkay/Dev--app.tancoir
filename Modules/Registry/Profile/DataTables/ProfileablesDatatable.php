...
namespace Modules\Registry\Profile\DataTables;

use Illuminate\Support\Facades\Auth;
use Yajra\DataTables\Services\DataTable;

class ProfileablesDatatable extends DataTable
{
    public function dataTable($query)
    {
        return datatables()
			...
			->addColumn('is_active', function ($profile) {				
				...
				if ($user->isProfileActive($profile)) {
					return "<uk-icon icon=\"check-check\" cls-custom='ms-2'></uk-icon>";
				}
				
				return view('core::components.datatable.profile_activator')->with([
					'activate' => route('app.profile.activate', [
